...
# milestone bits, arm slot B.  Everything here is undone by restoring
# /root/rung2/p7-env.orig over p7 and writing 0x14 back to the slot register.
set -e

PREBOOT='mw.l 0x02390028 0x10000000; mw.l 0x0484000c 0x2aea; mw.l 0x04840018 0x1; mw.l 0x04840018 0x0; mw.l 0x048700a8 0x80000; mw.l 0x04840000 0x1'
...
echo "=== zero the U-Boot pre-console buffer at 0x480e8000 ==="
dd if=/dev/zero of=/dev/mem bs=4096 seek=295144 count=2 2>&1 | tail -1 || echo "  (pre-console zeroing failed -- read with care)"
