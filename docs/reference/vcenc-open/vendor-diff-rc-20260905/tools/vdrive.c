 ...
 *
 * 2026-09-05 (RC campaign on real content): src=<file> feeds RECORDED raw YUYV
 * frames (WxH, stride == width, concatenated; recorded from the open V4L2
 * driver by v4lrec.c) instead of the synthetic card; loop=pingpong|wrap picks
 * the frame order once the clip is exhausted.  motion=vscroll|hscroll applies
 * a labelled software translation (mstep px per frame, wrap-around) to the
 * real frame inside the window mstart..mstop (the offset holds outside it);
 * motion=jump displaces the frame by mjump px (default H/2) every mstep frames
 * -- the window-switch stimulus (mstep=1 defeats single-reference prediction).
 * chg=<frame>:<kbps>[,<frame>:<kbps>...] changes the bitrate MID-STREAM through
 * the public AX_VENC_GetRcParam/SetRcParam API right before SendFrame of that
 * frame.  Every frame logs the source frame index + motion offset it used.
 *
...
/* real-content source + motion + mid-stream change */
static const char *SRC=NULL, *LOOP="pingpong", *MOTION="none", *CHG=NULL;
static int MSTEP=4, MSTART=0, MSTOP=-1, MJUMP=-1; /* motion=jump: every MSTEP frames offset += MJUMP (default H/2) */
static const uint8_t *srcmap=NULL; static int NSRC=0;
#define MAXCHG 16
static int chg_frame[MAXCHG], chg_br[MAXCHG], nchg=0;
static FILE *LOG;
...
static int src_open(void){
    int fd=open(SRC,O_RDONLY); if(fd<0){P("!! src open %s failed",SRC);return -1;}
    struct stat st; fstat(fd,&st); size_t fsz=(size_t)W*H*2; NSRC=(int)(st.st_size/fsz);
    if(NSRC<1){P("!! src too small (%lld bytes < one %dx%d YUYV frame)",(long long)st.st_size,W,H);close(fd);return -1;}
    srcmap=mmap(NULL,(size_t)NSRC*fsz,PROT_READ,MAP_PRIVATE,fd,0); close(fd);
    if(srcmap==MAP_FAILED){P("!! src mmap failed");srcmap=NULL;return -1;}
    P("src=%s frames=%d loop=%s motion=%s mstep=%d window=%d..%d",SRC,NSRC,LOOP,MOTION,MSTEP,MSTART,MSTOP);
    return 0;
}
/* frame index -> source frame (pingpong: 0 1 .. n-1 n-2 .. 1 0 1 ..; wrap: i mod n) */
static int src_index(int i){
    if(NSRC==1) return 0;
    if(!strcmp(LOOP,"wrap")) return i%NSRC;
    int per=2*(NSRC-1), k=i%per; return k<NSRC?k:per-k;
}
/* compose frame i into dst: the recorded frame, translated by the running motion offset */
static int moff=0;
static void compose(uint8_t *dst, int i){
    const uint8_t *s = srcmap + (size_t)src_index(i)*W*H*2;
    int in_win = i>=MSTART && (MSTOP<0 || i<MSTOP);
    if(in_win && !strcmp(MOTION,"jump")){ if(i>MSTART && ((i-MSTART)%MSTEP)==0) moff += (MJUMP>0?MJUMP:H/2); }
    else if(in_win && strcmp(MOTION,"none")) moff += MSTEP;
    size_t row=(size_t)W*2;
    if(!strcmp(MOTION,"vscroll") || !strcmp(MOTION,"jump")){
        int o=((moff%H)+H)%H;
        for(int y=0;y<H;y++) memcpy(dst+(size_t)y*row, s+(size_t)((y+o)%H)*row, row);
    } else if(!strcmp(MOTION,"hscroll")){
        int o=(((moff/2)%(W/2))+(W/2))%(W/2)*2; size_t ob=(size_t)o*2; /* macropixel-aligned: keeps YUYV phase */
        for(int y=0;y<H;y++){ const uint8_t *r=s+(size_t)y*row; uint8_t *d=dst+(size_t)y*row;
            memcpy(d, r+ob, row-ob); memcpy(d+row-ob, r, ob); }
    } else memcpy(dst, s, row*(size_t)H);
}
static void parse_chg(void){
    if(!CHG) return; char buf[256]; strncpy(buf,CHG,sizeof buf-1); buf[sizeof buf-1]=0;
    for(char *t=strtok(buf,","); t && nchg<MAXCHG; t=strtok(NULL,",")){ char *c=strchr(t,':'); if(!c) continue; *c=0; chg_frame[nchg]=atoi(t); chg_br[nchg]=atoi(c+1); nchg++; }
    for(int k=0;k<nchg;k++) P("chg plan: frame %d -> %d kbps",chg_frame[k],chg_br[k]);
}
/* mid-stream bitrate change through the public RC-param API (the only knob the open controller must mirror) */
static void apply_chg(int chn, int i){
    for(int k=0;k<nchg;k++) if(chg_frame[k]==i){
        AX_VENC_RC_PARAM_T rp; memset(&rp,0,sizeof rp); AX_S32 g=AX_VENC_GetRcParam(chn,&rp);
        unsigned before = (!strcmp(RC,"cbr")||!strcmp(RC,"cvbr")) ? rp.stH264Cbr.u32BitRate : rp.stH264Vbr.u32MaxBitRate;
        if(!strcmp(RC,"cbr")) rp.stH264Cbr.u32BitRate=chg_br[k];
        else if(!strcmp(RC,"vbr")||!strcmp(RC,"avbr")) rp.stH264Vbr.u32MaxBitRate=chg_br[k];
        else if(!strcmp(RC,"cvbr")) rp.stH264CVbr.u32MaxBitRate=chg_br[k];
        AX_S32 s=AX_VENC_SetRcParam(chn,&rp);
        AX_VENC_RC_PARAM_T rp2; memset(&rp2,0,sizeof rp2); AX_VENC_GetRcParam(chn,&rp2);
        unsigned after = (!strcmp(RC,"cbr")||!strcmp(RC,"cvbr")) ? rp2.stH264Cbr.u32BitRate : rp2.stH264Vbr.u32MaxBitRate;
        P("chg frame=%d br %u -> %d kbps: GetRcParam=0x%x SetRcParam=0x%x readback=%u mode=%d", i, before, chg_br[k], g, s, after, (int)rp2.enRcMode);
        BR=chg_br[k];
    }
}
...
int main(int argc, char **argv){
    ...
    for(int i=1;i<argc;i++){
        ...
        else if(kv(a,"src",&v)) SRC=v; else if(kv(a,"loop",&v)) LOOP=v; else if(kv(a,"motion",&v)) MOTION=v;
        else if(kv(a,"mstep",&v)) MSTEP=atoi(v); else if(kv(a,"mstart",&v)) MSTART=atoi(v); else if(kv(a,"mstop",&v)) MSTOP=atoi(v);
        else if(kv(a,"mjump",&v)) MJUMP=atoi(v);
        else if(kv(a,"chg",&v)) CHG=v;
...
#define RCM(x) (H265 ? AX_VENC_RC_MODE_H265##x : AX_VENC_RC_MODE_H264##x)
    ...
    for(int i=0;i<NFRAMES;i++){
        int srcf=-1, off=0;
        if(srcmap){ srcf=src_index(i); compose(card,i); off=moff; for(size_t k=0;k<size;k+=8) *(uint64_t*)(virt+k)=*(uint64_t*)(card+k); }
        else if(i==0 || MOVE){ fill(card,i); for(size_t k=0;k<size;k+=8) *(uint64_t*)(virt+k)=*(uint64_t*)(card+k); }
        apply_chg(CHN,i);
        fi.stVFrame.u64PTS=(AX_U64)i*1000000ULL/FPS; fi.stVFrame.u64SeqNum=i+1;
        if(srcmap) P("src frame %d: srcf=%d off=%d br=%d", i, srcf, off, BR);
