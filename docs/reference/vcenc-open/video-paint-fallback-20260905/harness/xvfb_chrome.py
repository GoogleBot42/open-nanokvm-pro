...
async def main():
    ...
    async with websockets.connect(page["webSocketDebuggerUrl"], max_size=None) as ws:
        ...
        for cp in checkpoints:
            ...
            extra_js = os.environ.get("PROBE_JS")
            if extra_js:
                rx = await call("Runtime.evaluate", expression=extra_js, awaitPromise=True, returnByValue=True)
                print("probe-js:", json.dumps(rx.get("result", {}).get("value", rx))[:2000], flush=True)
