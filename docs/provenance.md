...
| `/opt/swupdate/bin/swupdate` | ~500K | vendor OTA binary; its `S99checkota` call is commented out. We replaced it, and since #86 retired that too -- the appliance updates by system bundle (`nanokvm-update`) |
...
| `…/nanokvm_pro_sys_<ver>.tar.gz` → 302 → `objects.githubusercontent.com` | user clicks update, or the (default-off) `nanokvm.update.auto` timer fires | **APPROVE** — our update asset: the appliance's whole store closure plus its kernel, ~460 MB, SHA-512 verified against the manifest. **Both callers use these two URLs and no others** — the button routes through `nanokvm-update`, and the URLs are set in exactly two places, `nanokvm.update.{stableUrl,previewUrl}` (`nixos/appliance.nix`) and `updateBaseUrl` (`flake.nix`). |
