...
`maix_ax620e_sdk` — the SPL source, `imgsign` and the two FDL download agents.
**Since #95 there is no prebuilt binary in that list at all**: `ax_gzip` is
gone.
...
**None of them is a binary either, since #95** (on hardware 2026-09-12).
`ax_gzip` — an Axera x86-64 static ELF with no source, which packed every stage
the SPL loads into the "axgzip" LZ77 the SoC's gzipd block decompresses — was
the last one. The SPL is compiled `SUPPPORT_GZIPD=FALSE` now and reads each
stage straight from flash to its load address, so `atf` and `uboot` are stored
raw behind their signed headers and nothing compresses them.
`.#checks.<sys>.no-x86-blobs` asserts it over the DEFAULT chain and the DEFAULT
flashable image: 5 store paths, 37 files, 3 ELFs, all `EM_AARCH64`, no file
named `ax_gzip`, and none of those packages still declaring itself
`x86_64-linux`-only. `pkgs/boot.nix` deletes `tools/ax_gzip_tool` from its own
build tree, and no other file in the repo names it.
