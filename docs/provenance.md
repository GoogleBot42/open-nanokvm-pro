...
|---|---|---|---|
| **aic8800 WiFi/BT firmware** | `/opt/firmware/aic8800/*.bin` (~3.5 MB), loaded by our from-source `aic8800_{bsp,btlpm,fdrv}.ko` (built from the SDK kernel tree into `/usr/lib/modules`, udev-autoloaded; `/opt/scripts/wifi.sh` `modprobe`s by name — `pkgs/rootfs/wifi.sh`) | closed firmware, redistributable | **APPROVED 2026-09-04 (Jeremy, #28): WiFi stays, and the wireless firmware is the ONLY closed content permitted on the image — no closed userspace, no closed `.ko`, ever.** It executes on the radio, not the CPU. If it does not survive the mainline move (#26), WiFi is dropped, not the policy. |
...
| 1 | `spl` | `spl_…_signed.bin` | `pkgs/boot.nix` (written last, deliberately). **The one closed payload still on the eMMC: the vendor sign tool splices `build/tools/imgsign/eip_ax620e.bin` — byte-identical to the U-Boot `eip130_firmware[]` array, 78528 B — into the SPL *package* at `fw_flash_addr` 0xCC00 and `fw_bak_flash_addr` 0x2CC00, and the signed `spl_header` declares its `fw_size`/`fw_check_sum`.** That is a BootROM contract (`spl_AX620E_sign.py`, `-fw` in `boot/bl1/{spl,sd}/Makefile`), not a build option: the SPL code itself is clean (`spl_…nanokvm.bin` scans blob-free). Pinned by the #90 assertion at exactly those two offsets so it cannot grow silently. **`.#nixos-firmware-image-mainline` stores `pkgs/spl-minimal.nix`'s SPL instead** -- the same bl1 sources rebuilt for the GPT layout (#89 rung 4), with OP-TEE and the ddrinit partition compiled out; it carries the same two spliced copies, because that splice is a BootROM contract rather than a build option. **`.#spl-minimal-noeip` is the experiment that would drop it**: the identical SPL signed with an EMPTY `-fw` member, so `fw_size` and `fw_check_sum` are 0 and no firmware bytes are spliced at all (build-asserted: zero copies). Whether the ROM boots that is the open question, and getting it wrong costs an AXDL trip, so it is a gated hardware step and not the default. |
