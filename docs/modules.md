...
instantiated from that host's package set. That used to be forced — the vendor's
`ax_gzip` packer is an x86-64-only static ELF and every boot-chain stage went
through it — and since #95 it is not: nothing in a default build runs a prebuilt
binary. The *system* the modules describe is `aarch64-linux`, built here through
binfmt/qemu-user.
