...
### aic8800 (WiFi/BT) — WORKING (#85, 2026-09-11)
...
directions. The part on this board is an **AIC8801** (SDIO vid 0x5449, did
0x0145, read at probe 2026-09-11), which the driver maps to the `SDIO/aic8800`
firmware directory by itself. See "ON HARDWARE: THE RADIO SCANS" below for what
the three hardware rounds found.
...
12. **#85 WiFi: aic8800 out-of-tree module + firmware pin** — **DONE, and the radio scans on hardware (2026-09-11).** Offline half
    `radxa-pkg/aic8800` pinned and patched
...
13. **#86 Flake-based updates replace the custom OTA / #100 nix on the device**
    ...
    nix in the sandbox. **Hardware done the same day**: the bootstrap, four
    generations installed from a signed cache and booted, an unattended
    idle-gated reboot and a forced rollback onto the previous generation — see
    "What exists now (#100)". What is left is not #100's: the cache itself
    (**#96**, needs-human) and a release that actually publishes
    `nanokvm_pro_sys_latest.json`, without which the web UI's button 404s before
    it reaches the updater. [updates.md](updates.md).
...
### What exists now (#100, 2026-09-11) — ON HARDWARE: NIX, AND AN UPDATE FROM A CACHE

The board has nix, its store is registered, and it has installed four
generations from a signed binary cache — one of them taken by the idle timer
with nobody watching, one rolled back by `bootcount` onto the generation before
it. **Eight boots, 51-58 s each from `reboot` to SSH, `bootcount` `0xB0010001`
at every health gate.** Nothing was written as a block device; the recovery
mechanism in the way was the one under test.

The cache is a stand-in until #96: a `file://` binary cache signed with a
throwaway `nix key generate-secret` key, served by `python3 -m http.server` on
the build host and reached over an **SSH reverse tunnel**, so the device's
configured cache was `http://127.0.0.1:8099/cache`.
`.#appliance-toplevel-cachetest` — the shipped appliance with the channel,
cache, key and version taken from `builtins.getEnv`, and byte-identical to
`.#appliance-toplevel` under pure evaluation — is the reusable half of that.

#### The bootstrap

Generation 4 was a `tar`-installed appliance with no nix at all. **73 missing
store paths, 23.3 MB gzipped**, md5-verified after `drop_caches`, extracted into
`/nix/store`; `system-5-link` written by hand; `switch-to-configuration boot`;
reboot; then `nix-store --load-db` of a registration covering the **774-path**
union closure of the new toplevel and the one both boot configs named.

| oracle | value |
|---|---|
| boot | 51 s, `bootcount` `0xB0010001` → cleared by `nanokvm-mark-good` |
| `nix --version` | `nix (Nix) 2.34.8` |
| `nix path-info -r /run/current-system \| wc -l` | 748 |
| `nix-store --verify --check-contents` | clean, exit 0 |
| `df /boot` | 51 MB of 245 MB — unchanged; the kernel did not change |

**`/nix/store` is a read-only bind mount on the appliance.** `tar -C /nix/store`
fails with exit 2 and nothing useful on stderr until
`mount -o remount,rw /nix/store`; put it back with `remount,bind,ro`, because
`remount,ro` alone is a silent no-op on a bind. `nanokvm-update` already does
both around its `nix copy`; a hand bootstrap has to do it itself. (nix as root
needs no help — it unshares a mount namespace of its own.)

#### The update, the idle gate, the rollback

- **An untrusted NAR does not install.** `error: cannot add path … because it
  lacks a signature by a trusted key`, exit 1, nothing in the store, profile
  unmoved.
- **`nix copy` is a delta.** A version-bump generation fetched **22 paths of
  748**; the next one, **9**.
- **The idle gate held both ways.** With a `curl` on `/api/stream/mjpeg`,
  `nanokvm-update update` installed and refused the reboot
  (`"busy":["stream","video","web"],"stream_clients":1`), writing both markers.
  Ten minutes after the stream was dropped, `nanokvm-update-reboot` logged
  `"idle":true,…"video_quiet_sec":1087` and took it **unattended**.
- **A forced rollback landed on the previous generation**, with a real pending
  update in place: `/run/booted-system` = generation 5, the profile symlink =
  generation 6, `bootcount` `0xB001000B` at the gate, and
  `nanokvm-update reboot-if-idle` reporting "update 2.1.0-test2 was installed
  but 2.1.0-alpha.6 is running -- it did not survive the boot".
- **Collection is safe and real.** `gc` (keep 3) freed **64 paths / 132.1 MiB**,
  including the `tar`-era generations and an orphaned closure — confirming on
  hardware that `nix-collect-garbage` deletes **unregistered** paths.
  `gc --keep 1` then printed `gc: keeping generation 5 -- a boot config names
  it` and freed **26 paths / 27.9 MiB** from the generation nothing named.
  `nix-store --verify --check-contents` clean after each.

**`gc` leaves the extlinux menus stale.** Neither config is rewritten by the
collector, so a `LABEL` can name a generation whose `init=` no longer exists —
harmless, because both `DEFAULT` entries are exactly what `gc` pinned. The next
...
before `install()` and fetches the manifest from the URL **compiled into the Go
binary** (`updateBaseUrl`), not from `nanokvm.update.stableUrl`; no release
publishes `nanokvm_pro_sys_latest.json` yet. Everything past that 404 is proven:
...
Proven with a throwaway key, removed afterwards: 7 paths in 4.5 s, valid in the
board's database on arrival. The `kvm-device` skill's recipe works as written;
it needs **key** auth, because nix drives `ssh` itself.

#### End state

Generation 9 = `.#appliance-toplevel` from `main` (2.1.0-alpha.6), booted;
generations 5 and 9 in the profile, every test generation collected; both
`DEFAULT` entries resolve to it; `nix-store --verify --check-contents` clean;
748 paths, 1.3 GB of store, 1.4 GB of 29 GB root used, 51 MB of 245 MB `/boot`;
`bootcount` `0xB0010000`; `nanokvm.service` active, web 200.
`nanokvm-display` is `inactive` on purpose — `ConditionPathExists=/dev/fb0`, and
there is no panel driver on mainline yet.

**What is NOT proven.** A real cache (#96): the attic endpoint, its TLS, key
custody and `attic push` from a runner. A release cut from a tag, and therefore
the manifest asset existing at `releases/latest/download` — which is what the
web UI's button is waiting for. An update that changes the **kernel**: every
generation here shared one, so `/boot` never grew and `configurationLimit = 3`
has still never held three kernels. A generation that genuinely fails to boot —
...
*(The offline half. Three of the decisions below were wrong on hardware --
see "ON HARDWARE: THE RADIO SCANS" further down, which is the current truth.)*
...
### What exists now (#85, 2026-09-11) — ON HARDWARE: THE RADIO SCANS

**`iw dev wlan0 scan` returned 64 networks, and `/kvmcomm/scripts/wifi.sh
try_scan` — the script NanoKVM-Server's WiFi page actually execs — returned 38
of them as JSON the server can parse.** The chip runs closed firmware from the
path compiled into `aic8800_bsp.ko`, `wlan0` is a cfg80211 interface, and a
bare `wpa_cli -i wlan0 status` answers. Joining a network is the only thing
left, and that needs credentials (see the end of this section).

**The part is an AIC8801**, read off the SDIO bus at probe:

```
aicbsp: aicbsp_sdio_probe:1 vid:0x5449  did:0x0145
aicbsp: aicbsp_sdio_probe:2 vid:0x544A  did:0x0146
rwnx_load_firmware :firmware path = /run/current-system/firmware/aic8800_fw/SDIO/aic8800/fw_patch_table_u03.bin
... fw_adid_u03.bin, fw_patch_u03.bin, fmacfw.bin, fmacfw_patch.bin
aicbsp: bt patch version: - Jul 23 2025 14:21:09 - git 83c2573
```

`0x5449/0x0145` is `SDIO_VENDOR_ID_AIC8801`/`SDIO_DEVICE_ID_AIC8801`, so the
per-chip firmware patch picked `SDIO/aic8800/` and the `_u03` revision files on
its own — which is the whole reason all five chip directories ship and no
module parameter names one. Narrowing the package to `aic8800/` would save
3.9 MB of the 5.1; it is deliberately **not** done, because one board is not
evidence about a batch and the cost is 0.3 % of the system closure.

It also proves the two firmware decisions offline reasoning could only assert:
the compiled-in path `/run/current-system/firmware/aic8800_fw/SDIO` resolves
through `hardware.firmware` at runtime, and the files are found under their own
names — i.e. the `compressFirmware = false` opt-out is doing its job, because
this driver `filp_open`s a literal path and a `fmacfw.bin.zst` would simply not
exist to it.

#### Three faults, and not one of them was visible offline

**1. `CONFIG_RESET_GPIO=m` meant the SDIO host never probed at all.** The first
boot had no `mmc2` in `/sys/class/mmc_host` — so no bus scan, no card, and an
`aic8800_bsp` whose power-on timed out against a chip nothing had powered:

```
platform wifi-pwrseq: deferred probe pending: pwrseq_simple: reset control not ready
platform 104d0000.mmc: deferred probe pending: supplier wifi-pwrseq not ready
aicbsp: aicbsp_set_subsys, fail to set AIC_WIFI power state to 1
AICWFDBG(LOGERROR)	rwnx_mod_init, set power on fail!
```

`mmc_pwrseq_simple_probe` counts the `reset-gpios` phandles, and when there is
**exactly one** it asks the reset core for a reset *control* first, falling
back to plain GPIOs only if that returns NULL. The core's answer turns on
`if (!IS_ENABLED(CONFIG_RESET_GPIO)) return optional ? NULL : ...` — and
**`IS_ENABLED()` is true for `=m`**, which is what arm64 defconfig gives. So
the core synthesised an auxiliary `reset-gpio` device out of our `reset-gpios`
property and waited forever for a driver in a module this kernel does not
build, cannot find and has no search path for. Permanent `-EPROBE_DEFER`, and
because `104d0000.mmc` is a consumer of the pwrseq it inherited it.

The fix is one line, `CONFIG_RESET_GPIO=y`, and the general rule is worth more
than the line: **this kernel has no module search path** — six `.ko` are
installed by hand and nothing else can ever load — so **any `=m` symbol that a
built-in `IS_ENABLED()` tests is a lie**. Make it present or make it absent;
never leave it modular. Nothing offline can catch this: the config is valid,
the DT is valid, every package builds, and the failure is two `deferred probe
pending` lines 12 seconds into a boot.

**2. A radio that is not there must not make the KVM unhealthy.** The unit
`exit 1`-ed when `wlan0` failed to appear. That made `systemctl
is-system-running` report `degraded`, which made `nanokvm-mark-good` poll for
240 s and give up, which left `bootcount` uncleared — so **every reboot counted
as a failed boot attempt and the fourth would have rolled the board onto the
fallback generation**, over a missing radio on a device whose HDMI, USB and
ethernet all worked. `nanokvm-wifi.service` now never fails: every dead end is
a journal line and `exit 0`, and it logs whether `104d0000.mmc` is in
`/sys/class/mmc_host` *before* it tries anything, because that single fact
separates "the module/firmware/chip is at fault" from "nothing powered it".
`wifi.sh` matches — `try_scan` prints `[]`, `connect_stop` and `ap_stop`
succeed, `connect_start` fails naming the missing interface.

Proven on the board with the radio still absent: with the unit no longer
failing, `mark-good` went `healthy after 0s`, cleared `bootcount` to
`0xB0010000` and promoted the generation to the fallback.

**3. `wpa_cli`'s control path is nixpkgs', not upstream's.** #85 shipped
`userControlled = false` plus an explicit `ctrl_interface=/run/wpa_supplicant`,
reasoning that a bare `wpa_cli -i wlan0` — which is exactly what the server
runs — defaults to `/var/run/wpa_supplicant`. On the board it failed twice
over: `/run/wpa_supplicant/client: No such file or directory`, and then
`Failed to connect to non-global ctrl_ifname: wlan0`. `strings` on the board's
own binary settles it:

```
/run/wpa_supplicant/client
/run/wpa_supplicant/control
```

Both paths are patched in by nixpkgs, and `networking.wireless.userControlled`
is the option that creates both. It is on now, and the `extraConfig` override
is gone. **Measure the binary; do not reason about its defaults** — this is the
same lesson as the browser-codec rule in CLAUDE.md, in a different subsystem.

#### The scan, end to end

`iw dev wlan0 scan` counted 64 SSIDs across 2.4 and 5 GHz. `wifi.sh try_scan`
produced 3970 bytes of JSON, 38 entries, every `bssid` well-formed and every
`signal`/`frequency` numeric — checked with `jq` after applying the server's
own `fixInvalidJSONEscapes` transformation, which is needed because a hidden
SSID comes back from `wpa_cli` as `\x00\x00...` and that is not valid JSON.
That is not a bug in either half: `wpa_cli`'s `printf_encode` emits `\\`, `\"`,
`\n`, `\r`, `\t` (all valid JSON escapes) and `\xNN` for the rest, and the
server decodes `\xNN` itself. The script passes the SSID through byte for byte
precisely so that fixer still works.

#### Left open

- **Joining a network: `needs-human`.** It needs an SSID and a password.
  Everything up to association is proven; the remaining test is Jeremy at the
  WiFi page with his own network, watching for `wpa_state=COMPLETED`, a DHCP
  lease on `wlan0` and the UI reporting connected.
- **AP mode** is still not configured (see the module header).
- **Bluetooth** is still not built, by choice.
- **Narrowing the firmware** to `SDIO/aic8800/` alone, now that the part is
  known, is a 3.9 MB saving nobody has asked for.
...
### What exists now (rung 2j, 2026-09-08) — the tuning window is 34 wide, and the read still fails

Six hardware rounds; full detail in
[`RUNG2J.md`](reference/mainline/uboot-mainline-20260908/RUNG2J.md), the sweeps
in [`tune-map-20260908.txt`](reference/mainline/uboot-mainline-20260908/tune-map-20260908.txt).

The hypothesis was an edge-of-window tuning pick: U-Boot chooses HRS06 tune 17
where Linux chooses 15, and a value that passes one 128-byte CMD21 block and
fails a 1 MiB CMD18 is what a marginal sampling phase looks like. Instrumenting
`sdhci_cdns_execute_tuning()` to print its own pass/fail map disproves it
outright:
...
Tuning points 0–31 (or 0–33) all pass; only the top few of the 40 fail, and the
pick is the exact centre — **15 in the second run, Linux's own value**. The 1 MiB
environment read timed out anyway, with a register table byte-identical to the
run that picked 16. And these passes are real transfers: `mmc_send_tuning()`
reads 128 bytes through the same sdhci/ADMA path and `memcmp()`s the result
against the expected pattern.

> **A 128-byte single-block read succeeds at 32–34 of 40 sampling phases; a
> 2048-block read fails at every one of them.** Phase, PHY delay, tuning and
> clock are all excluded.

Two fixes tried and rejected, both recorded rather than kept:

- **Dropping `CONFIG_MMC_HS200_SUPPORT`** so U-Boot settles at `MMC_HS_52` like
  the SPL (verified compiled out: no `MMC_SUPPORTS_TUNING` in the generated
  config, no `execute_tuning` in the ELF). The board went dark — but see below,
  a dark board proves nothing here.
- **`ADMA_MAX_LEN` 65532 → 65536.** U-Boot chunks ADMA descriptors at 65532
  bytes, which is not a multiple of 512, so every intermediate boundary in a
  transfer longer than that lands 508 bytes inside a block; Linux has always
  used 65536, which is block aligned. The theory fit the size dependence
  perfectly and changed nothing. Reverted — an unproven behaviour change is a
  confound for the next rung.

**Hangs on this board are intermittent.** The same image, in two consecutive
rounds, hung once and reset cleanly once, with the environment read failing
identically in both and therefore the same built-in `bootcmd` running down the
same path. A dark board is not evidence about the change under test; two of six
rounds went to learning that.

**A watchdog does not arm from U-Boot.** The obvious answer to an unobservable
hang is to make it a reset: slot B's bootable bit is already consumed, so the
reset lands on slot A and Linux reads the buffer back. `board_early_init_r()`
(before `initr_env()`) was given the exact sequence our own `ax630c_wdt` uses —
24 MHz source, TORR `0xD693` for 300 s, TORR_LOAD strobe, magic-word kick,
WDT_EN. A control run with the arm in place and the rest known-good returned
normally with a full console, so the **arm site is harmless**; a hung round with
the same arm sat dark for ten minutes without resetting, so the **dog never
runs**. The kernel driver takes clocks *and resets* from the peripheral clock
controller before touching the block; U-Boot enables neither, so the writes are
posted into a block that is not clocked. Rung 2e's `mw`-only "watchdog reset"
was, on this reading, something else entirely. **Releasing that clock and reset
is the prerequisite for making any future rung's hang observable.**

What is left is **V4 mode**, now the only structural difference between a driver
that reads this eMMC and one that does not — and the only candidate that
explains a failure which depends on transfer size. Linux sets `HOST_CONTROL2`
bits 12 and 13 and runs 64-bit ADMA2 descriptors; mainline U-Boot has no V4
support at all.
...
### What exists now (rung 2p, 2026-09-08) — the FDT lands correctly; the retry has to re-init the card

Five rounds against a budget of four. Detail in
[`RUNG2P.md`](reference/mainline/uboot-mainline-20260908/RUNG2P.md), consoles in
[`rung2p-fdt-20260908.txt`](reference/mainline/uboot-mainline-20260908/rung2p-fdt-20260908.txt).
Row 2 is still not done: the kernel starts and does not reach the network.

**The FDT fix works.** Patch `0018` sets `fdt_high` and `initrd_high` to
`0x5f000000` in the built-in environment, and the device tree now lands at
`0x5effa000` — under the ceiling, inside the `mem=512M` window — against
`0x7e68c000` in rung 2o. Placement no longer depends on `ram_top`.

**Retries only work if they re-initialise the card.** The same patch wraps the
payload reads in three attempts, and that is not enough: the console shows two
`Error reading cluster` failures then a success on a **1375-byte** file — three
single blocks. The failures are bursty because a `DATA_TIMEOUT` poisons this
controller and U-Boot's per-command `SDHCI_RESET_CMD`/`_DATA` does not clear it
(rung 2k). Putting **`mmc rescan`** in front of each attempt does clear it, and
with it the boot read `extlinux.conf`, the 48.8 MiB kernel and the dtb and
handed off — slot register `0x30000015`, bits 28 and 29, no failure bit, so
`sysboot` never returned. **That belongs in the built-in `bootcmd`; it is
currently only in the stored environment.**

**`mem=512M` cannot simply be dropped.** Removing it left the board dark for
seventeen minutes *with no watchdog reset*, where the identical configuration
with it reset itself at 337 s every time — and a hang that also defeats WDT0 is
the AXI-stall signature on this SoC. Every `reserved-memory` node is inside the
first 512 MB, so whatever is above it is not described in the device tree; a
TrustZone/TZASC region would not be. The appliance really is running on half its
RAM (`free -m` total **428 MB**) and that is worth fixing, but rung 3 should find
where the vendor U-Boot injects the argument and what it protects first.

**What still fails.** With the FDT correct and the payload loaded, the kernel
starts and then either dies before ramoops probes (watchdog node disabled +
`clk_ignore_unused`: reset at 337 s, `/sys/fs/pstore` empty) or goes dark and
stays dark (stock dtb: `ax630c_wdt` stops WDT0 at probe). The second is *further
along*, not worse — but the two evidence channels are mutually exclusive as
things stand, and that is the thing to design around next.

**One round was lost to the environment**, and the mechanism is worth knowing:
`Unknown command 'axera_boot'`. **The vendor U-Boot on slot A rewrites
`bootcmd=axera_boot` and saves it on every boot**, so any run that arms slot B
without re-setting the environment inherits it and mainline U-Boot sits at its
prompt until the watchdog fires.

Device left on slot A, register `0x00000014`, `uboot_b` restored byte-for-byte
(p6 `1521dc39f8a50e726c708fde2c8edce2`), environment vendor-clean, `/boot` back
to `ver` alone, `checkboot` `Result=success`, `nanokvm` active, web 200.

### What exists now (rung 2q, 2026-09-09) — row 2 is DONE

Six rounds. Detail in
[`RUNG2Q.md`](reference/mainline/uboot-mainline-20260908/RUNG2Q.md); the whole
slot-B boot log in
[`rung2q-slotb-console-20260909.txt`](reference/mainline/uboot-mainline-20260908/rung2q-slotb-console-20260909.txt).

**The NixOS appliance boots the whole mainline chain — mainline BL31, mainline
U-Boot, mainline kernel — and answers SSH in 153 s.** Oracles, from the slot-B
system itself: slot register `0x30000018` (bits 28 + 29, slot B, no failure
bit), `psci: SMC Calling Convention v1.5` against the vendor BL31's v1.2,
`/proc/cmdline` matching the `APPEND` verbatim, `Active PHY interface:
RGMII (1)`, `PHY [stmmac-0:01] driver [RTL8211F Gigabit Ethernet]`,
`Link is Up - 1Gbps/Full`, `free -m` total 428 MB, `systemctl is-system-running`
= `running`, and a `systemctl reboot` that lands back on slot A unattended.
`fw_printenv bootcount` is still `not defined`.

**Rung 2p's central inference was wrong.** It read an empty `/sys/fs/pstore`
and concluded the kernel died before ramoops probed. `/sys/fs/pstore` is empty
on a *healthy* boot too: `systemd-pstore.service` archives every record into
`/var/lib/systemd/pstore` and unlinks it about ten seconds in, under fixed
names the next boot overwrites. Read in the right place, the log said the
kernel had been booting to systemd all along.

**Two real bugs, both invisible under the vendor loader.**

1. `dwmac-axera` pulsed the EMAC block reset and *then* wrote the flash syscon's
   PHY interface select. **The MAC samples that select when the reset is
   released.** The vendor-derived U-Boot has an ethernet driver and leaves it at
   RGMII (`+0x28` = `0x600`) already, so the ordering never showed; mainline
   U-Boot prints `Net:   No ethernet found.` and leaves it at 0, and the MAC came
   out of reset believing it was on RMII.
2. The fourteen RGMII pads — `RGMII_MDCK`, `RGMII_MDIO` and the twelve data
   pads — were named by **no** pinctrl group and claimed by **no** GPIO
   consumer, so Linux never programmed them and inherited whatever the loader
   left. The vendor loader writes mux 0, drive 15; mainline U-Boot writes
   nothing. `gmac_pins` in the board DT fixes it for every loader.

The general rule is worth more than either instance: **a pad that no DT node
names is a pad your port does not own.** It works exactly as long as the
bootloader you happened to test under keeps programming it, which is precisely
the assumption a mainline port breaks. This is the SW_PWR trap one bus over.

**The harness needed two fixes to say any of this.** `watchdog.open_timeout=N`
on the command line: `ax630c_wdt` adopts U-Boot's running dog and, with
`CONFIG_WATCHDOG_HANDLE_BOOT_ENABLED=y` and `CONFIG_WATCHDOG_OPEN_TIMEOUT=0`,
the core pets it forever from kernel context because nothing in the appliance
opens `/dev/watchdog`. That is the whole of "dark and stays dark"; with an open
deadline a failed boot resets itself at ~4 minutes and the log survives. And the
ramoops console zone moved to `0x480d4000` and grew to `0x8000`, because a whole
...
### What exists now (rung 3b, 2026-09-09) — SLOT A KEEPS IT

**Row 3 is done.** Slot A runs mainline BL31 + mainline U-Boot + extlinux and
stays there. Rung 3 had proven the chain boots and lost it two boots in three;
this round found why a lost boot could not recover, fixed it twice over, and
measured the result.

Full write-up and both console captures:
`docs/reference/mainline/uboot-mainline-20260908/RUNG3B.md` and
`rung3b-console-20260909.txt`.

#### The two patches

**`0021` — never offer the card a signal voltage the board cannot drive.**
`mmc_set_lowest_voltage()` offers `MMC_SIGNAL_VOLTAGE_330` and nothing else for
every mode below `MMC_DDR_52`. On this board that offer cannot be taken and
cannot report that it was not taken: `sdhci_set_voltage()` prints
`failed to set vqmmc-voltage to 3.3V` and returns without touching Host
Control2, and the host half of the switch goes through `->set_control_reg()`,
which returns void. The mode is then configured with the controller signalling
at 1.8 V and the core believing 3.3 V, the confirming ext_csd read fails, and
the walk moves down — nine candidates, nine refusals, `unable to select a mode:
-5`, no block device. The fix asks the vqmmc regulator's constraints what
levels it can reach (`mmc_vqmmc_signal_voltages()`), intersects the allowed
mask with them, and — because that alone would leave a 1.8 V-only board with no
mode below `MMC_DDR_52` — offers 1.8 V for the backwards-compatible and
high-speed timings when the supply cannot reach 3.3 V. EXT_CSD CARD_TYPE bits 0
and 1 name no voltage, so that is legal; and on a board that can do 3.3 V the
intersection masks the addition straight back out, so nothing changes there.
`mmc_vqmmc_below_330()` (patch `0010`, the initial state) is now one line on top
of the same helper, so init and re-init decide the I/O level in one place.

**`0022` — retry a failed block read before giving up.** `mmc_bread()` treated
one failed transfer as a failed read of the whole request, and everything above
it treats a failed read as a failed boot. `sdhci_send_command()` already resets
the command and data lines on its way out, so the next attempt starts from a
clean controller. Three attempts per chunk, `mmc_poll_for_busy()` in between,
and a `mmc%d: retrying read at <lba>` warning so a recovered failure is still
visible instead of silently absorbed.

Both are written against upstream and shaped for submission.

#### The measurement: 10/10

Ten consecutive slot-B boots of the appliance through the mainline chain, slot A
left on the vendor-derived chain so any failure had somewhere to land. None did.
Every boot reached SSH, `systemctl is-system-running` = `running`, register
`0x30000029`. Wall time to SSH 171–211 s, of which ~110 s is the boot chain.

Twelve read failures across roughly a million single-block commands — about 1.2
per 100 000 — and **every one recovered by the first retry**; no LBA appears
twice. Seven were U-Boot's own 10 s software timeout in
`sdhci_transfer_data()`, worth ~10 s of boot each; five were an immediate
`SDHCI_INT_ERROR`. They are bursty: three boots carried all twelve, seven
carried none. **Not one re-initialisation happened in the ten**, so the ten
boots do not exercise `0021` at all — that proof is the probe boot below.

Every init, in every boot, lands on the same place:
`mmc: selected mode 12, 8-bit, 50000000 Hz, signal 2` — `MMC_HS_400_ES`,
8-bit, 50 MHz, 1.8 V signalling.

#### The probe boot, and what it says about #91

`.#uboot-mainline-probe` is the tee image plus a `preboot` that lifts the
`cdns,single-block-only` cap for exactly one command, reads the card's own state
with CMD13 **before any CMD12 and before the controller is reset**, reads 8 MiB
in `MMC_HS` and 8 MiB in HS200, and forces a re-init. One boot, three answers.

**The card starts; the host goes deaf. That reverses #91's headline.** On an
open-ended CMD18 — no CMD23, no Auto CMD12 — `INT_STATUS` is `0x00000000` (no
error interrupt at all; the failure is the software timeout waiting for
`DATA_END`), `PRESENT_STATE` is `0x014f0236` with `DAT_LINE_ACTIVE` and
`READ_TRANSFER_ACTIVE` **set** and DAT[3:0] = 0x4, and CMD13 answers
`0x00000b00` — **CURRENT_STATE 5 = DATA**. Three attempts, three identical
answers. #91 records the opposite ("all eight DAT lines high, no transfer
active, state 4 = TRAN, it never started") because that CMD13 came *after* the
controller reset, which is exactly what returns the card to TRAN. The rare
CMD17 failure has the same host-side shape (`READ_TRANSFER_ACTIVE` still set,
usually no error interrupt) with the card already back in TRAN — so CMD18 fails
this way always and CMD17 fails this way rarely, and it is one bug.

**Mode dependence is not answerable at that sample size.** `MMC_HS` and HS200
each read 16 384 blocks with zero retries, but at 1.2e-5 per read that expects
0.2 failures, so 0 and 0 discriminate nothing. A real comparison needs of the
order of 10⁶ single-block reads per arm — about 35 minutes of bus time each,
which does not fit one boot under a 300 s watchdog stage. The harness is in
`.#uboot-mainline-probe`; only the read counts need raising.

**HS200 tuning fails on this controller.** Six `CMD21` attempts returned
`ERROR | DATA_TIMEOUT` and the core selected HS200 anyway, which then read
8 MiB cleanly. Banked on #91; nothing depends on it, because the board runs
HS400ES.

**`0021` is hardware-proven.** Five separate re-initialisations happened in that
one boot and the console carries no `failed to set vqmmc-voltage to 3.3V` and no
`unable to select a mode`. The forced re-init lands on `mode 12, 8-bit,
50000000 Hz, signal 2` — byte for byte the first init's line.
...
consecutive warm reboots, each `0x30000015` (preboot + extlinux, no failure bit,
slot A re-armed by `nanokvm-checkboot`), `systemctl --failed` empty, web 200,
`/proc/cmdline` the extlinux APPEND.
...
clean. So a switch is `nix copy --to ssh://root@<board>` (key auth — nix drives
`ssh` itself) followed by `nanokvm-update install-toplevel <path>`, and an
update is `nanokvm-update update` against a cache; the hand `tar` bootstrap in
the `kvm-device` skill is history for this board. **`/nix/store` is a read-only
bind mount** — flip it with `mount -o remount,rw /nix/store` and back with
`remount,bind,ro` before writing it by hand. `nanokvm.update.cacheUrl` is empty
on the generation the board runs, so it polls nothing and installs nothing until
#96; point a test board at a cache with `.#appliance-toplevel-cachetest`
([updates.md](updates.md) "Local testing").
