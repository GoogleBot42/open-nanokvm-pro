...
pkgs.runCommand "nanokvm-update-idle"
{
  ...
  # A /boot of the shape NixOS's extlinux builder writes (#99). The updater
  # never touches it -- `switch-to-configuration boot` owns it, and the stub
  # above is a `true` -- but `status` reads it, and its presence is what makes
  # "the updater left /boot alone" a statement with something behind it.
  ...
  LABEL nixos-default
    ...
    LINUX ../nixos/${oldSys}-kernel
    INITRD ../nixos/${oldSys}-initrd
    APPEND init=/nix/store/${oldSys}/init root=/dev/loop0p5 panic=10
    FDT ../nixos/${oldSys}-dtbs/ax630c-nanokvm-pro.dtb
  EOF
  sed -i 's/^  //' "$R/boot/extlinux/extlinux.conf"
  cp "$R/boot/extlinux/extlinux.conf" "$R/boot/extlinux/extlinux-fallback.conf"
  cp "$R/boot/extlinux/extlinux.conf" "$PWD/boot-before.txt"
  ...
  mkdir -p "$B/store" "$SRV/api/update"
  ...
  # NO boot/ HALF (#99): the kernel, the initrd and the dtb are store paths
  # inside the closure, and the bootloader builder is what puts them on /boot.
  ...
  # #99: the only writer of /boot is the bootloader builder that
  # `switch-to-configuration boot` runs, and here that is a `true` stub -- so
  # after a full install the boot config must be byte-identical.
  cmp -s "$PWD/boot-before.txt" "$R/boot/extlinux/extlinux.conf" \
    || fail "the updater wrote /boot/extlinux/extlinux.conf"
  ok "in use: installed, both markers written, NO reboot, /boot untouched"
