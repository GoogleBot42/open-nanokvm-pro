...
{
  config = {
    # NO ATX GPIO UNIT AT ALL, and that is the #81 result rather than a gap.
    # The 4.19 image had one: it poked the VI_D7 pad mux with `devmem` and
    ...
    # programs the pad. The tool that does the requesting is `nanokvm-gpio`
    # below, and the server reaches it by absolute store path
