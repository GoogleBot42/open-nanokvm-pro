...
Outputs (all overwritten in place):
  ...
  pkgs/vcenc-ewl/tests/vcenc_hevc_vectors.h   vendor HEVC fixed-QP programs (sparse; #64)
"""
...
HEVC = os.path.join(REF, "vendor-diff-hevc-20260905")
...
def collect_qp():
    ...
    hevc = sorted(glob.glob(os.path.join(HEVC, "H3", "vendor_fixqp*_f000.txt"))
                  + glob.glob(os.path.join(HEVC, "H3", "vendor_fixqp*_f001.txt")))
    HEVC_P_VARIANT = {20, 26, 27, 33}
    for p in files + hevc:
        v = load(p)
        is_hevc = p.startswith(HEVC)
        if is_hevc:
            hp = p.endswith("_f001.txt")
            t = "I"                     # HEVC I and P share the I table (H3)
        else:
            hp = False
        ...
        for k in range(8):
            idx = q - 2 * k
            val = v[125 + k]
            if hp and idx in HEVC_P_VARIANT:
                continue                # HEVC P-variant entry, not the I table
        ...
        if hp:
            continue    # HEVC P sw37 carries the P-variant F lo16 (q=34); L is I-only here
...
# --------------------------------------------------------------------------
# 4. Vendor HEVC fixed-QP programs (#64): the 1080p ladder (per-frame
#    snapshots f000 = IDR, f001 = first P) and the fixed-QP32 golden vectors
#    at six more geometries (vendor-diff-hevc-20260905/H3, H3x).
# --------------------------------------------------------------------------

RE_H3X = re.compile(r"vendor_fixqp32_(\d+)x(\d+)_IDR\.txt$")


def emit_hevc(path):
    progs = []
    for tag, qpi, qpp in LADDER:
        for kind, qp, fn in (("IDR", qpi, "f000"), ("P", qpp, "f001")):
            p = os.path.join(HEVC, "H3", "vendor_fixqp%s_%s.txt" % (tag, fn))
            v = load(p)
            progs.append(("fixqp%s" % tag, kind, 1920, 1080, qp, v[7] >> 26, v, rel(p)))
    for p in sorted(glob.glob(os.path.join(HEVC, "H3x", "vendor_fixqp32_*x*_IDR.txt"))):
        m = RE_H3X.search(p)
        w, h = int(m.group(1)), int(m.group(2))
        for kind in ("IDR", "P"):
            pp = p.replace("_IDR.txt", "_%s.txt" % kind)
            v = load(pp)
            progs.append(("fixqp32_%dx%d" % (w, h), kind, w, h, 32, v[7] >> 26, v, rel(pp)))
    with open(path, "w") as o:
        o.write("""/* SPDX-License-Identifier: GPL-2.0 OR MIT */
/* AUTO-GENERATED by pkgs/vcenc-ewl/tests/gen_vectors.py -- do not edit.
...
""")
        for tag, kind, w, h, qp, pic, v, src in progs:
            name = "VHQ_%s_%s" % (tag, kind)
            o.write("/* %s */\nstatic const vhq_reg %s[] = {\n" % (src, name))
            for r in range(1, NREGS + 1):
                if v[r]:
                    o.write("    { %3d, 0x%08x },\n" % (r, v[r]))
            o.write("};\n")
        ...
        for tag, kind, w, h, qp, pic, v, src in progs:
            name = "VHQ_%s_%s" % (tag, kind)
            o.write('    { "%s/%s", %d, %d, %d, %d, %d, %s, sizeof %s / sizeof %s[0] },\n'
                    % (tag, kind, w, h, 1 if kind == "P" else 0, qp, pic, name, name, name))
        o.write("};\n")
    return progs
...
def main():
    ...
    hq = emit_hevc(os.path.join(HERE, "vcenc_hevc_vectors.h"))
    print("vcenc_hevc_vectors.h: %d programs" % len(hq))
